...
let lastVerificationTimestamp = parseInt(process.env.LAST_VERIFICATION_TIMESTAMP);
let notifies;
...
const saveConfig = (timestamp) => {
    try {
        ...
        data = data.replace('LAST_VERIFICATION_TIMESTAMP=' + lastVerificationTimestamp, 'LAST_VERIFICATION_TIMESTAMP=' + timestamp);
...
setInterval(() => {
    ...
    notifies?.forEach(async (notify) => {
        if (notify && notify?.transID != 0) {
            ...
            const orders = await Order.find({ "phoneNumber": { "$regex": `${last8digits}`, "$options": "i" } }).sort({updatedAt: 1});
            if (orders?.length) {
                for (let i = 0; i < orders.length; i++) {
                    const product = await Product.findById(orders[i].productId);
                    if (amount > 0) {
                        const orderPay = parseInt(orders[i]?.amount) - parseInt(orders[i]?.paid);
                        if (amount >= orderPay) {
                            orders[i].status = true;
                            orders[i].paid += orderPay;
                            amount -= orderPay;
                            console.log(product.quantity - orders[i].amount / product.price);
                            product.quantity = product.quantity - orders[i].amount / product.price;
                        } else {
