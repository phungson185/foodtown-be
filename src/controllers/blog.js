...
const getBlogs = async (page, limit, filter, name) => {
  try {
    const blogs = await Blog.find(
      name ? { name: { $regex: `${name.toUpperCase()}` } } : null
    )
      .skip((page - 1) * limit)
      .limit(limit)
...
const getCommentsBlog = async (id, page) => {
  try {
    ...
    const result = blog?.comments.splice(
      3 * parseInt(page),
      blog.comments.length - 3 * parseInt(page) < 3
        ? blog.comments.length - 3 * parseInt(page)
        : 3
    );
    return result;
  } catch (error) {
...
const dislikeBlog = async (userId, blogId) => {
  try {
    const blog = await Blog.findById(blogId);
    if (
      !blog.likes.find((like) => like.user.toString() === userId.toString())
    ) {
